...
public class ItemDAO extends DataBaseInfo {
	...
	public void purchaseInsert(PurchaseDTO dto) {
		con = getConnection();
		sql = " insert into purchase(purchase_num, purchase_date"
				+ ", purchase_price, delivery_addr, delivery_addr_detail"
				+ ", delivery_post, delivery_phone, message, purchase_status"
				+ ", member_num, delivery_name, purchase_name) "
	...
	public void cartMerge(CartDTO dto) {
		...
		sql += " on ( c.goods_num = g.goods_num and member_num = ? ) ";
		sql += " when MATCHED THEN ";
		sql += " update set cart_qty = cart_qty + ? ";
		sql += " when not MATCHED THEN ";
		sql += " insert (member_num, goods_num, cart_date, cart_qty) ";
		sql += " values(?, ?, sysdate, ?) ";
	...
	public void wishItem(String memberNum, String goodsNum) {
		...
		sql += " values( ?, g.goods_num, sysdate ) ";
