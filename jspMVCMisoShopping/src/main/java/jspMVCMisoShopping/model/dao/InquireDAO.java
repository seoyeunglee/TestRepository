...
public class InquireDAO extends DataBaseInfo {
	...
	public void inquireReplyUpdate(InquireDTO dto) {
		con = getConnection();
		sql = " update Inquire set inquire_answer = ?, emp_num = ? , inquire_answer_date = sysdate where inquire_num = ? ";
		try {
	...
	public void inquireUpdate(InquireDTO dto) {
		con = getConnection();
		sql = " update inquire set inquire_subject = ?, inquire_contents = ?, inquire_kind = ? where inquire_num = ? ";
		try {
			pstmt = con.prepareStatement(sql);
	...
	public void inquireSelectOne(InquireDTO dto) {
		...
		try {
			pstmt = con.prepareStatement(sql);
			// pstmt.setInt(1, Integer.parseInt(inquireNum));
